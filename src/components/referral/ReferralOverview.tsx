...
import { Users, HandCoins, UserPlus, ArrowRight } from "lucide-react";
...
export const ReferralOverview = ({ staffMemberId, onNavigate }: ReferralOverviewProps) => {
  ...
  const loadStats = async () => {
    try {
      ...
      // Client network earnings (as referrer)
      const { data: earned } = await supabase
        .from('referral_transactions')
        .select('commission_amount')
        .eq('referrer_creative_id', staffMemberId);
      
      const totalEarned = earned?.reduce((sum, tx) => sum + Number(tx.commission_amount), 0) || 0;
